...
function processBook(book, callback) {
    ...
    request.get(baseUrl + book + ".json", (err, res, body) => {
        ...
        if(data.authors) {
            ...
            data.authors.forEach(author => {
                authorsPromises.push(new Promise( (resolve, reject) => {
                    processAuthor(author.key, (data) => resolve(data));
                }));

            });

            Promise.all(authorsPromises).then( (data) => {
                data.forEach(processedAuthor => {
                    processedBook.authors.push(processedAuthor);
                });
                callback({key: book, data: processedBook});
            });
        }
        else {
            callback({key: book, data: processedBook});
...
function processAuthor(author, callback) {
...
request(options, (err, res, body) => {
    ...
    if(body.status == "ok") {
        ...
        let bookPromises = [];
        body.result.forEach(book => {
            bookPromises.push(new Promise( (resolve, reject) => {
                processBook(book, (data) => resolve(data));
            }));
        })
